...
void MCADCPWM3P_pwm_write(int16_t dutyU_q15, int16_t dutyV_q15, int16_t dutyW_q15)
{
    ...
    constexpr uint32_t U_MASK = (TIM_CCER_CC1E | TIM_CCER_CC1NE);
    constexpr uint32_t V_MASK = (TIM_CCER_CC2E | TIM_CCER_CC2NE);
    constexpr uint32_t W_MASK = (TIM_CCER_CC3E | TIM_CCER_CC3NE);
    constexpr uint32_t ALL_MASKS = (U_MASK | V_MASK | W_MASK);
    ...
    if(dutyU_q15 > 0){
        ...
        ccer_shadow |= U_MASK;
        ccmr1_shadow &= ~OC1M_MASK;  // Clear OC1M bits
        ...
        ccer_shadow &= ~U_MASK;
        // Note: For negative duty, we don't need to modify CCMR since outputs are disabled
    }
    ...
    if(dutyV_q15 > 0){
        ...
        ccer_shadow |= V_MASK;
        ccmr1_shadow &= ~OC2M_MASK;  // Clear OC2M bits
