...
uint16_t MCADCPWM3P_regScanData[2][MCADCPWM3P_REGSCAN_CHANNEL_NUM * MCADCPWM3P_REGSCAN_DMABUFFER_DEPTH];
...
void MCADCPWM3P_adcReg_dmaInit(const MCADCPWM3P_Config &cfg)
{
    ...
    LL_DMA_SetDataLength(MCADCPWM3P_DMA_INSTANCE, MCADCPWM3P_DMA_REGSCAN_STREAM, 
                        MCADCPWM3P_REGSCAN_CHANNEL_NUM * MCADCPWM3P_REGSCAN_DMABUFFER_DEPTH);
...
uint16_t MCADCPWM3P_adcReg_read(uint16_t ch)
{
    ...
    uint32_t sum = 0;
    for (int i = 0; i < MCADCPWM3P_REGSCAN_DMABUFFER_DEPTH; i++) {
        uint32_t dataIndex = i * MCADCPWM3P_REGSCAN_CHANNEL_NUM + ch;
        sum += MCADCPWM3P_regScanData[readBuffer][dataIndex];
    }
    
    uint16_t average = sum / MCADCPWM3P_REGSCAN_DMABUFFER_DEPTH;
    return average;
}
