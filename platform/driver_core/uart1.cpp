...
static uint8_t uart1_dma_uart1_tx_buffer[UART1_TX_DMA_BUFFER_SIZE];
static uint8_t uart1_dma_rx_buffer[UART1_RX_DMA_BUFFER_SIZE];
...
bool uart1_write(uint8_t* data , uint8_t len);
...
uint8_t uart1_rx_buf[UART1_RX_DRV_BUFFER_SIZE];
uint8_t uart1_tx_buf[UART1_TX_DRV_BUFFER_SIZE];
...
{
    // DMA interrupts
    NVIC_SetPriority(DMA2_Stream5_IRQn, 4);
